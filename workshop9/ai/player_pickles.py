...
class Player(PlayerAI):
    def __init__(self):
        super(Player, self).__init__('Pickles')

    def update(self, player_positions, player_lives, food_positions):
        if (not(hasattr(self, 'valoare'))):
            self.valoare = 1
        ...
        for playerIndex, playerPos in enumerate(player_positions):
            if not(playerPos[0] == self.position[0] and playerPos[1] == self.position[1]):
                ...
                playerDistances.insert(foodIndex, distance)
        ...
        if (distX + self.position[0] - diameter <= 0):
            distX = diameter - self.position[0]
        if (distX + self.position[0] + diameter >= 800):
            distX = 800 - self.position[0] - diameter

        if (distY + self.position[1] - diameter <= 0):
            distY = diameter - self.position[1]
        if (distY + self.position[1] + diameter >= 800):
            distY = 800 - self.position[1] - diameter

        if (self.previousTarget[0] - radius <= distX + self.position[0] <= self.previousTarget[0] + radius and self.previousTarget[1] - radius <= distY + self.position[1] <= self.previousTarget[1] + radius):
            self.valoare += 1
